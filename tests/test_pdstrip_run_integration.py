...
def test_metadata_from_fixture(parsed_components) -> None:
    ...
    assert np.allclose(metadata.section_x, np.array([-4.0, -3.0, -2.0, -1.0, -0.0, 1.0, 2.0, 3.0, 4.0]))
...
def test_notebook_relevant_heave_roll_selection(parsed_folder) -> None:
    ...
    heave = dataset["added_mass"].integrate("section").sel(influenced_dof="Heave", radiating_dof="Heave")
    roll = dataset["added_mass"].integrate("section").sel(influenced_dof="Roll", radiating_dof="Roll")
